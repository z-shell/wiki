...
`for` 语法是最受欢迎、更简洁和更优化的。 单个命令的工作原理与 [标准语法][standard-syntax] 调用相同。
...
上述单一命令安装了 6 个插件（[git extension][2] 包），其中包括所有插件通用的基础 ices`as "null" wait "3" lucid` 和 6 个插件专用的附加ice-modifiers。
