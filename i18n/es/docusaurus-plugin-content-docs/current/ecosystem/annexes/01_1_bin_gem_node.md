...
> [!] shims created by BGN annex have a fixed structure, this option instructs ZI to show the list of shims that results from the `sbin''` ice of the loaded plugins. If a plugin has `sbin'git-open'`, for example, then this means that there has to be such shim already created.
