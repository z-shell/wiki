...
- Docusaurus site with three independent docs content roots:
  - `docs/` → `/docs` (Zi product documentation only)
  - `community/` → `/community` (cross-project material that benefits the wider community)
  - `ecosystem/` → `/ecosystem` (installable or companion ecosystem components such as annexes, plugins, packages, tools, and tests)
...
- Place pages by subject, not only audience: Zi itself belongs in `docs/`; broadly useful community material belongs in `community/`; installable or companion ecosystem components belong in `ecosystem/`.
- Prefer the wiki as the canonical home for durable organization-wide docs; keep individual repos lean by linking to the wiki or using generated/synchronized copies instead of duplicating long-form guidance.
