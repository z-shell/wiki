...
The machine needs <kbd>zsh</kbd>, <kbd>git</kbd>, and <kbd>curl</kbd> or <kbd>wget</kbd>. Fetch the installer to a file first. A failed download then stops with a non-zero exit instead of handing an empty script to <kbd>sh</kbd>, and the file can be checked against the published [checksum][checksum-txt] before it runs:
