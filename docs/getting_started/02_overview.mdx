...
## Plugin and snippet loading {/* #plugin-and-snippet-loading */}
...
Using `curl`, `wget`, etc. allows us to almost completely avoid code dedicated to Oh-My-Zsh and Prezto, and also to other frameworks. It delivers better performance as has a low footprint on memory and shorter loading times.
...
## About as"program" {/* #about-asprogram */}
...
## Ice modifier: atpull'…' {/* #ice-modifier-atpull */}
...
This way the `mv` ice can be used to induce permanent changes into the plugin's contents without blocking the ability to update it with `git` in the case of snippets.
...
## Ice modifier: subscribe'…' {/* #ice-modifier-subscribe */}
...
## Snippets as'…' program {/* #snippets-as-program */}

Commands can also be added to `$PATH` using **snippets**:

```zi {2} showLineNumbers
...
Snippets also support `atpull`. There’s also an `atinit` ice-modifier, executed before each loading of plugin or snippet.

:::

## Snippets as'…' completion {/* #snippets-as-completion */}
...
## The completion management {/* #the-completion-management */}
...
### Listing available completions {/* #listing-available-completions */}
...
### Enabling / disabling - completions {/* #enabling--disabling---completions */}
...
You probably won't load the prompt in such a way, however, it is a good example in which turbo mode can be observed. The exclamation mark causes Zi to reset the prompt after loading the plugin – commonly needed for themes. Using `zsh-users/zsh-autosuggestions` without any drawbacks:
...
### Turbo mode is the key to the performance {/* #turbo-mode-is-the-key-to-the-performance */}
...
### Turbo mode & lucid {/* #turbo-mode--lucid */}
...
## Turbo mode with sophisticated prompts {/* #turbo-mode-with-sophisticated-prompts */}
...
```text title="print $precmd_functions"
...
The exclamation mark in `atload'!…'` is to track the functions allowing the plugin to be unloaded, as described in the [standard syntax guide](/docs/guides/syntax/standard#atclone-atpull-atinit-atload). It might be useful for the multi-prompt setup described next.

### Summary of turbo mode {/* #summary-of-turbo-mode */}
...
## Automatic condition based - load & unload {/* #automatic-condition-based---load--unload */}
...
### A Glance at the prompts {/* #a-glance-at-the-prompts */}
...
```zi {1} title="~/.zshrc" showLineNumbers
...
```zi {2} {3} title="~/.zshrc" showLineNumbers
