...
## <span className="heading-with-icon"><Icon name="plug" /> Plugin and snippet loading</span>
...
## <span className="heading-with-icon"><Icon name="github" style="brands" /> Oh-My-Zsh, Prezto</span> {#oh-my-zsh-prezto}
...
## <span className="heading-with-icon"><Icon name="bolt" /> Snippets and Performance</span>
...
## <span className="heading-with-icon"><Icon name="wand-magic-sparkles" /> Ice Modifiers</span>
...
## <span className="heading-with-icon"><Icon name="code" /> The completion management</span>
...
### <span className="heading-with-icon"><Icon name="list" /> Listing available completions</span>
...
<SvgExternal height='455.91' width='1660' url='/img/cast/svg/zi_clist.svg' alt='Zi completion list' />

To show more completions per line by providing an **argument** to `clist`, e.g.: `zi clist 6`, will show:

<SvgExternal height='455.91' width='1660' url='/img/cast/svg/zi_clist_6.svg' alt='Zi completion list 6' />

### <span className="heading-with-icon"><Icon name="toggle-on" /> Enabling / disabling completions</span>
...
<SvgExternal height='455.91' width='1180' url='/img/cast/svg/zi_csearch.svg' alt='Zi completion search' />

## <span className="heading-with-icon"><Icon name="code-branch" /> The subversion for subdirectories</span>
...
## <span className="heading-with-icon"><Icon name="bolt" /> Turbo Mode</span> {#turbo-mode-zsh--53}
...
### <span className="heading-with-icon"><Icon name="key" /> Turbo mode is the key to the performance</span>
...
### <span className="heading-with-icon"><Icon name="bell" /> Turbo mode & lucid</span> {#turbo-mode--lucid}
...
## <span className="heading-with-icon"><Icon name="wand-magic-sparkles" /> Turbo mode with sophisticated prompts</span>
...
```shell title="print $precmd_functions" showLineNumbers
...
The exclamation mark in `atload'!…'` is to track the functions allowing the plugin to be unloaded, as described in the [documentation for `atclone`, `atpull`, `atinit`, and `atload` ice modifiers](/docs/guides/syntax/standard#atclone-atpull-atinit-atload). It might be useful for the multi-prompt setup described next.

### <span className="heading-with-icon"><Icon name="check-double" /> Summary of turbo mode</span>
...
Turbo mode with the empty `wait` ice will postpone the loading `1` ms after that, so `precmd` will not be called at the first prompt. This makes autosuggestions inactive at the first prompt.
...
## <span className="heading-with-icon"><Icon name="sync" /> Automatic condition based - load & unload</span> {#automatic-condition-based---load--unload}
...
<SvgExternal width='1100' height='325.65' url='/img/cast/svg/zi_load_at_tmp.svg' alt='Zi load at /tmp' />
...
<SvgExternal width='1100' height='325.65' url='/img/cast/svg/zi_load_not_tmp.svg' alt='Zi load not at /tmp' />
