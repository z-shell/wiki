...
description: A handbook covering native Zsh scripting techniques and best practices.
keywords:
  - zsh-handbook
---
...
### @ is about keeping array form {/* #-is-about-keeping-array-form */}
...
### Skipping dirname basename {/* #skipping-dirname-basename */}
...
Read more: [zsh: 14 Expansion](https://zsh.sourceforge.net/Doc/Release/Expansion.html#Modifiers).
...
#### Multi-line matching like with grep {/* #multi-line-matching-like-with-grep */}

Suppose you have a Git repository and want to check if it contains files not under version control. You could do this in Bash style like follows:
...
Those are 3 forks: for `git status`, for `echo`, and for `grep`. This can be solved by the `:#` substitution and the `(M)` flag described above in this section (just check if the number of matched lines is greater than 0). However, there's a more direct approach:

```shell showLineNumbers
local git_status="$(git status --porcelain)" nl=$'\n'
if [[ "$git_status" = *((#s)|$nl)\?\?* ]]; then
  echo found
fi
```
...
[[ "$(git status --porcelain)" = *((#s)|$nl)${~required_preceding}${needle}* ]] && echo found
```

It does a single fork (called `git` status). The `${~variable}` means (the `~` init): "the variable is holding a pattern, interpret it". All in all, instead of regular expressions we were using patterns (globs) (see [this section](#using-built-in-regular-expressions-engine)).

### Pattern matching in AND-fashion {/* #pattern-matching-in-and-fashion */}
...
### Ternary expressions with `+,-,:+,:-` substitutions {/* #ternary-expressions-with----substitutions */}
...
### Ternary expressions with `:#` substitution {/* #ternary-expressions-with--substitution */}
...
### Using built-in regular expressions engine {/* #using-built-in-regular-expressions-engine */}
...
### Code execution in `//` substitution {/* #code-execution-in--substitution */}
...
#### Tip: serializing with Bash {/* #tip-serializing-with-bash */}
...
### Testing for Git subcommand {/* #testing-for-git-subcommand */}
...
### Counting unquoted-only apostrophes {/* #counting-unquoted-only-apostrophes */}
...
### Parsing INI file {/* #parsing-ini-file */}
