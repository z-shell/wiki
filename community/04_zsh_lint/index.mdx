...
```sh
go install github.com/z-shell/zsh-lint/cmd/zsh-lint@v1.3.0
```
...
directories, or discover source files. Every explicit path is treated as Zsh
source. Only the `zsh-lint.json` project configuration is discovered
automatically; see [Project configuration](./03_project_configuration.mdx).
...
>
  Describe plugins and multi-profile projects in a `zsh-lint.json` that the CLI discovers automatically.
</Card>
...
The guides describe v1.3.0, which includes project configuration and automatic
`zsh-lint.json` discovery. Automatic source-file discovery and shebang-based
classification are not implemented; that design is tracked in
