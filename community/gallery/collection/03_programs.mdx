...
#### GH-R
```shell showLineNumbers
zi ice from'gh-r' as'program' mv'numbat* numbat' sbin'**/numbat(.exe|) numbat'
zi light sharkdp/numbat
```

#### GH

Advanced. Use if you care about [numbat modules](https://numbat.dev/doc/cli-installation.html#guidelines-for-package-maintainers).
```shell showLineNumbers
zi ice as'program' from'gh' \
  atclone'export NUMBAT_SYSTEM_MODULE_PATH=$ZI[PLUGINS_DIR]/sharkdp---numbat/numbat/modules; cargo install -f --path numbat-cli;' \
  atpull'%atclone' \
